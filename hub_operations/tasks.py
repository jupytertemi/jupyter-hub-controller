...
def get_cameras():
    ...
    cameras = model.objects.prefetch_related("camera_setting_zone")
    camera_data = []
...
def hub_reset_task():
    try:
        reset_result = control_service(
            action="start", service_name="jupyter-hub-reset.service"
        )
        if not reset_result:
            return "Hub reset fail."
        return "Hub reset successfully."
    except Exception as e:
        logging.error(f"Hub reset error: {e}")
        return "Hub reset fail."


@shared_task(bind=True)
def hub_auto_restart_cloudflare_token(self):
    logging.info("Hub auto-restart cloudflare turn")
    response = get_ice_server()

    if response == {}:
        logging.error("Get Invalid ICE data")
        return "Get Invalid ICE data"
