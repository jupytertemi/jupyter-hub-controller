...
touch /var/lib/jupyter/.reset-intent
...
# === PHASE 4.5: GOLD-IMAGE HYGIENE (2026-05-01) ===
# Wipes residual files that contain hub-specific identity or camera credentials.
# These were not in the original wipe list and would leak prior-hub identity into
# flashed clones if Seattle is ever snapshotted as the gold image.
echo "PHASE 4.5: Gold-image hygiene — wiping residual identity-leak files..."

# Auto-backups (env-backup.timer creates these every 6h, NOT cleaned previously)
rm -f /root/jupyter-hub-controller/.env.backup-auto-* 2>/dev/null && echo "  removed auto-backup .env files"
rm -f /root/jupyter-hub-controller/.env.pre-test-backup 2>/dev/null && echo "  removed .env.pre-test-backup"

# Stale .env from a previous hub-controller layout (different hub identity!)
rm -f /root/jupyter-hub-controller.env 2>/dev/null && echo "  removed stale /root/jupyter-hub-controller.env"
rm -f /root/jupyter-hub-controller-a/.env 2>/dev/null && echo "  removed stale /root/jupyter-hub-controller-a/.env"
rm -f /root/jupyter-hub-controller-a/local_env 2>/dev/null

# Stale container .env timestamped backups
rm -f /root/jupyter-container/.env.bak-* 2>/dev/null && echo "  removed container .env timestamped backups"

# Frigate config backups (camera RTSP creds — per-deployment)
rm -f /root/jupyter-container/frigate/config/config.yaml.bak* 2>/dev/null && echo "  removed Frigate config .bak files"
rm -f /root/jupyter-container/frigate/config/backup_config.yaml 2>/dev/null
rm -f /root/jupyter-container/frigate/config/backup.db 2>/dev/null

# MediaMTX config backups (camera RTSP creds)
rm -f /root/mediamtx/mediamtx.yml.bak* 2>/dev/null && echo "  removed mediamtx.yml backups"

# AI source code .bak files (per-hub experimental edits — should not propagate)
find /root/jupyter-container -maxdepth 3 -name "*.py.bak*" -delete 2>/dev/null
find /root/jupyter-container -maxdepth 3 -name "*.py.bak-pre-*" -delete 2>/dev/null

# docker-compose.yml backups (carry per-hub bind-mount edits)
find /root/jupyter-container -maxdepth 1 -name "docker-compose.yml.bak-*" -delete 2>/dev/null

# HAProxy config backups
find /root/jupyter-container/haproxy -name "*.bak*" -delete 2>/dev/null
find /root/jupyter-container/haproxy -name "*.broken-*" -delete 2>/dev/null

# Stale duplicate hub-controller directory (from a prior deploy / clone)
rm -rf /root/jupyter-hub-controller-a 2>/dev/null && echo "  removed /root/jupyter-hub-controller-a"

# .env.systemd files — regenerated by entrypoint, safe to wipe
rm -f /root/jupyter-hub-controller/.env.systemd 2>/dev/null

# === Argus monitoring identity reset (defense-in-depth for gold image) ===
# start_hub_controller.sh's S3 step re-fills these from DEVICE_NAME on next boot.
# Wiping here ensures clones don't carry master's slug in Argus configs.
if [ -f /opt/secureprotect/config/agent.json ]; then
  sed -i 's/"hub_name":"[^"]*"/"hub_name":""/' /opt/secureprotect/config/agent.json
  echo "  reset Argus agent.json hub_name"
fi
if [ -f /opt/secureprotect/config/scrape.yml ]; then
  ...
  echo "  reset Argus scrape.yml hub_name + location"
fi
if [ -f /etc/systemd/system/secureprotect-vmagent.service ]; then
  ...
  echo "  reset vmagent service label"
fi
if [ -f /etc/systemd/system/secureprotect-ip-reporter.service ]; then
  ...
  echo "  reset ip-reporter HUB_NAME"
fi
...
# Additional /root cruft from prior dev sessions / installers / macOS metadata
rm -f /root/cloudflared.log /root/env.zip /root/local_env 2>/dev/null
rm -f /root/reset_hub.sh.bak-* 2>/dev/null
rm -rf /root/__MACOSX 2>/dev/null
rm -rf /root/jupyter-ble-controller-a /root/jupyter-ble-controller-old 2>/dev/null
rm -rf /root/jupyter-hub-controller-b /root/jupyter-hub-controller-old 2>/dev/null
echo "  removed additional /root cruft"

# Voice-AI dev prototype carrying prior Halo device identity (HALO_DEVICE_NAME leak)
rm -rf /root/voice_ai_v3 /root/jupyter-container/voice_ai_v3 2>/dev/null && echo "  removed voice_ai_v3 dirs"

# Older static .env_backup (filename doesn't match auto-backup glob above)
rm -f /root/jupyter-hub-controller/.env_backup 2>/dev/null && echo "  removed jupyter-hub-controller/.env_backup"

# Per-hub experimental .bak files inside hub-controller source (Phase 4.5 didn't recurse here)
find /root/jupyter-hub-controller -maxdepth 4 \( -name "*.bak*" -o -name "*.backup" \) -delete 2>/dev/null && echo "  removed hub-controller .bak/.backup files"

# VSCode remote-dev cache (carries HUB_USER_ID from prior owner)
rm -rf /root/.vscode-server 2>/dev/null && echo "  removed /root/.vscode-server"

# Redundant identity backup (read by Phase 1.5 for recovery, never previously deleted —
# would leak full DEVICE_NAME/HUB_SECRET/DEVICE_SECRET/TUNNEL_TOKEN to every clone)
rm -f /root/.env.identity.bak 2>/dev/null && echo "  removed /root/.env.identity.bak"

# Shell history (WiFi PSKs, bearer tokens, debug commands)
rm -f /root/.bash_history /root/.lesshst /root/.zsh_history 2>/dev/null && echo "  removed shell history files"

# SSH known_hosts (fingerprint trail of prior outbound SSH targets)
rm -f /root/.ssh/known_hosts /root/.ssh/known_hosts.old 2>/dev/null && echo "  removed /root/.ssh/known_hosts*"

# Git config with safe.directory pins referencing per-hub paths
rm -f /root/.gitconfig 2>/dev/null && echo "  removed /root/.gitconfig"

echo "PHASE 4.5: gold-image hygiene complete"
...
# Extended 2026-05-07: per-customer AI persistence (face embeddings,
# vehicle calibrations, parcel state, voice prompts, etc.). Image is the
# code; these bind-mount dirs are pure runtime data. Build dirs are NOT
# wiped (they contain Dockerfiles).
for media_dir in "$COMPOSE_DIR/alarm_audio" \
                 "$COMPOSE_DIR/voice_ai_data" \
                 "$COMPOSE_DIR/voice_ai_prompts" \
                 "$COMPOSE_DIR/pilot_face_recognition_ai" \
                 "$COMPOSE_DIR/pilot_face_train_ai" \
                 "$COMPOSE_DIR/pilot_hey_jupyter_voice_ai" \
                 "$COMPOSE_DIR/pilot_parcel_theft_AI" \
                 "$COMPOSE_DIR/pilot_suggested_facesAI" \
                 "$COMPOSE_DIR/pilot_unusual_sounds_ai" \
                 "$COMPOSE_DIR/pilot_vehicle_ai" \
                 "$COMPOSE_DIR/loiterai"; do
    if [ -d "$media_dir" ]; then
...
# Extended 2026-05-07: include motioniq named volume + prune anonymous
# volumes. Anonymous volumes accumulate per-hub MQTT/EMQX state across
# rebuilds — orphan them on every offboard.
echo "Removing Docker volumes (postgres + redis + motioniq + anonymous)..."
for vol in jupyter-container_postgres-vlm \
           jupyter-container_redis_data \
           motioniq_motioniq_data; do
    sudo docker volume rm "$vol" 2>/dev/null || echo "  Volume $vol not found or already removed."
done
# Reap anonymous volumes (long-hash names) — these are leftover from
# previous container generations and not referenced by current compose.
sudo docker volume prune -f 2>/dev/null | tail -3 || true
...
# done
# NOTE: secureprotect-agent.service was a deprecated unit name that no longer exists.
# Real Argus units (secureprotect-vmagent, secureprotect-node-exporter,
# secureprotect-ip-reporter.timer) are already enabled at install time and start
# automatically on the Phase 9 reboot — restarting them here is redundant.
# We keep the two below because Phase 3 stops them and Phase 9 reboot doesn't
# guarantee re-enable if they were disabled mid-reset.
for svc in jupyter-identity-guard.timer \
           haproxy-heal.timer; do
    sudo systemctl enable "$svc" 2>/dev/null || true
    sudo systemctl restart "$svc" 2>/dev/null || true
    echo "  Enabled+Restarted: $svc"
