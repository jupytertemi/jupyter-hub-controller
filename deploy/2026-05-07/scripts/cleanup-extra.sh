...
# 2026-05-07: extend with /tmp + docker build cache prune
# /tmp accumulates ad-hoc scratch (test tarballs, debug .py files, etc.)
# Anything older than 7 days is fair game.
TMP_DELETED=$(find /tmp -maxdepth 2 -type f -mtime +7 2>/dev/null | wc -l)
find /tmp -maxdepth 2 -type f -mtime +7 -delete 2>/dev/null || true
log "pruned $TMP_DELETED /tmp files (>7d)"

# docker build cache — accumulates from local image rebuilds (FaceAI, VehicleAI etc.)
# `docker system prune` does NOT touch buildx cache. Reaped >7d builds.
docker builder prune -f --filter "until=168h" >/dev/null 2>&1 || true
log "pruned docker build cache >7d"
